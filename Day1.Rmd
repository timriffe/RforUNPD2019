...
Spreadsheets are sort of a headache, and there are many competing methods in `R` to grab data from particular cell ranges and tabs. These are often OS dependent, so it's hard to recommend a single method for everyone.
...
Also, `fread()` from the `data.table` is also great.
