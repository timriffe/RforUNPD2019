...
## tidy data
*Tidy* data refers to tables of a particular layout, with one observation per row and one variable per column. 

### Example of tidy data
For example, you're probably all familiar with HMD lifetables: This data is tidy. If you were to take all the HMD single age lifetables and stack them, adding new grouping columns for Sex and Country, then this would still be tidy. That is, a single HMD observation consists in a unique combination of Country, Sex, Year, and Age. Each lifetable column is a variable, so the tidy way to store them is the traditional way: in columns. 
...
Different data structures exist for various reasons, but in the end tidy data facilitates the kind of thinking and operations needed for flexible data analyses and visualization.

## the gapminder `data.frame`
Let's work with a tidy `data.frame`. These are like a rectangular spreadsheet, or like a dataset in `Stata`: `data.frame`s have rows and columns, and there can be different kinds of data between columns, but only one kind of data within a column. Load it in case you didn't before:
...
In the *tidyverse*, we mostly work with `data.frames` or data-frame-like objects. 
```
