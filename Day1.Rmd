...
Here are a selection of functions that read in selected data formats.
### csv
`read_csv()`
### txt
### SPSS, SAS, STATA
...
### flat files
`read_fwf()` 
`fread()` from the `data.table` is great.
```{r}
library(gdata)
write.fwf(as.data.frame(gapminder), file = here("data","gapminder_flat.txt"))

read_fwf(here("data","gapminder_flat.txt"),
		 col_positions = fwf_empty(here("data","gapminder_flat.txt")),
